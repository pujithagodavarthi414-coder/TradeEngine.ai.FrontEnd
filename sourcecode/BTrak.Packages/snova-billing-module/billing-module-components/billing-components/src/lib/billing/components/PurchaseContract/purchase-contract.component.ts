...
import { PurchaseContractComponentPopup } from "./purchase-contract-popup.component";
import { Router } from "@angular/router";
@Component({
    selector: "app-billing-component-purchase-contract-list",
    templateUrl: "purchase-contract.component.html"
})
export class PurchaseContractListComponent extends AppBaseComponent implements OnInit {
    ...
    constructor(private BillingDashboardService: BillingDashboardService, private imagePipe: FetchSizedAndCachedImagePipe,private router: Router,
    ...
    this.getContracts();
        ...
        //     || (this.datePipe.transform(contract.contractDateFrom,"dd-MMM-yyyy").toString().toLowerCase().indexOf(this.searchText) > -1)
        //     || (this.datePipe.transform(contract.contractDateTo,"dd-MMM-yyyy").toString().toLowerCase().indexOf(this.searchText) > -1)
        // ));
        // this.contractList = temp;
        // this.contractListData = {
        //     data: this.contractList,
        //     total: this.contractList.length > 0 ? this.contractList[0].totalCount : 0,
        // }
    }
    ...
    editContractPopupOpen(rowData) {
        const dialogRef = this.dialog.open(PurchaseContractComponentPopup, {
            ...
            width: '600px',
            disableClose: true,
            data: { rowData: rowData, isEdit:true }
        });
    ...
    selectedRow(event) {
        this.router.navigate(["billingmanagement/shipmentExecution", event.contractId]);
    }
}