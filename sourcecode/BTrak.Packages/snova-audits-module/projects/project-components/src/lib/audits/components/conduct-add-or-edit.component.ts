...
@Component({
    selector: "conduct-add-or-edit",
    templateUrl: "./conduct-add-or-edit.component.html",
    changeDetection: ChangeDetectionStrategy.OnPush
})

export class ConductAddOrEditComponent {
    @Output() closeConduct = new EventEmitter<string>();
    @Output() editedConductData = new EventEmitter<any>();

    @Input("editConductData")
    set _editConductData(data: any) {
        if (data) {
            this.editConduct = true;
            this.editData = data;
            this.auditName = data.auditName;
            this.auditId = data.auditId;
            this.conductId = data.conductId;
            if (this.editData.isIncludeAllQuestions) {
                this.disableConduct = false;
                this.isCompleted = false;
                this.specificQuestionsIncluded = false;
            }
            else {
                this.unselectQuestionsCount = 1;
            ...
            this.addConductForm.patchValue(data);
        }
    }
    ...
    set _selectedAudit(data: any) {
        if (data.value) {
            ...
            this.auditName = data.value.auditName;
            this.auditId = data.value.auditId;
            this.initializeConductAddEditForm();
            this.addConductForm.get('auditConductName').disable();
            this.addConductForm.controls["responsibleUserId"].setValue(data.value.responsibleUserId);
            this.selectedMemberId = data.value.responsibleUserId;
        }
    }
    ...
    @Input("parentConductId")
    set _parentConductId(data: any) {
        if (data) {
            this.parentConductId = data;
        }
    }
    ...
    public initSettings = {
        plugins: "advlist,link,table,preview,charmap,lists,paste",
        branding: false,
        auto_focus: true,
    ...
    constructor(private _formBuilder: FormBuilder, private route: ActivatedRoute, private store: Store<State>, private actionUpdates$: Actions, private cdRef: ChangeDetectorRef,
        private employeeService: EmployeeService, private auditService: AuditService,) {
    ...
    addNewConduct() {
        ...
        this.conduct.responsibleUserId = this.selectedMemberId;
        if (this.editConduct) {
        ...
        this.conduct.isNewConduct = this.editConduct ? false : true;
        if (this.fromMain)
            this.conduct.fromConductComponent = true;
        if (this.parentConductId)
            this.conduct.parentConductId = this.parentConductId;
