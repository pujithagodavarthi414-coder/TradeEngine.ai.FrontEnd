...
@Component({
  selector: 'app-certifed-shs-jambi',
  templateUrl: './certifed-shs-jambi.component.html',
  styleUrls: ['./certifed-shs-north-sumatra.component.scss'],
  encapsulation: ViewEncapsulation.None
})

export class CertifiedSHFsJambiComponent extends AppBaseComponent implements OnInit {
    public kendoData = [];
        ...
        this.livesService.getKPI1CertifiedSHFsLocation(searchData).subscribe((response: any) => {
            ...
            if(response.success) {
                if(response.data.length > 0) {
                    var data1 = {category: "Total SHF's in Phase 01", value: response.data[0].totalSHFPhase1, color: "#FFEFBD"};
                    this.kendoData.push(data1);
                    data1 = {category: "SHF's Certified", value: response.data[0].shfCertified, color: "#FFC000"};
                    this.kendoData.push(data1);
