...
@Component({
  selector: 'app-ffb-productivity-north-sumatra',
  templateUrl: './ffb-productivity-north-sumatra.component.html'
})

export class FFBProductivityNorthSumatraComponent extends AppBaseComponent implements OnInit {
    xAxisCategories: any[];
    categorySeries: any = [];
        ...
        this.livesService.getffbProductivity(searchData).subscribe((response: any) => {
            ...
            if(response.success) {
                if(response.data.length > 0) {
                    ...
                    console.log("Data", this.xAxisCategories, this.categorySeries);
                } else {
