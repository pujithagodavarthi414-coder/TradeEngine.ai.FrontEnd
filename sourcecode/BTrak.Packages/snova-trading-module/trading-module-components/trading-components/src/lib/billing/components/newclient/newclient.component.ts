...
export class NewclientComponent extends AppBaseComponent implements OnInit {
  ...
  selectedContractTemplateIds: string[];
  ...
  refreshForm: boolean = true;
  reSendKycLoadingForm: boolean;
  isSGTrader: boolean = false;
  user: any;
  ...
  ngOnInit() {
    ...
    if (this.user.clientId == null) {
      this.isSGTrader = true;
    } else {
      this.isSGTrader = false;
    }
  ...
  viewClientDetails() {
    ...
    this.BillingDashboardService.getClients(clientDetails)
      .subscribe((responseData: any) => {
        ...
        this.cdRef.detectChanges();
        let formJson = this.selectedClientDetails.formJson ? JSON.parse(this.selectedClientDetails.formJson) : Object.assign({}, this.basicForm);
        var inputFormJson = formJson;
  ...
  getUploadObjects(obj, key, val, newVal, list) {
    ...
    for (var i in obj) {
      ...
      if (typeof obj[i] == 'object') {
        ...
        obj.properties['referenceTypeId'] = this.selectedClientId;
  ...
  editClientDetails() {
    ...
    this.BillingDashboardService.getClients(clientDetails)
      .subscribe((responseData: any) => {
        ...
        this.selectedClientTypeName = this.selectedClientDetails.clientTypeName;
        // if (this.selectedClientType == "Ship Broker") {
        //   this.clientForm.controls["brokerageValue"].setValidators([Validators.required]);
        //   this.clientForm.get("brokerageValue").updateValueAndValidity();
        // } else if (this.selectedClientType != "Ship Broker") {
        //   this.clientForm.controls["brokerageValue"].setValidators([]);
        //   this.clientForm.get("brokerageValue").updateValueAndValidity();
        // }
        ...
        if (this.rolesList && (roleids.length == this.rolesList.length)) {
  ...
  selectcoutry(country) {
    ...
    // this.clientForm.get("businesCountryCode").patchValue(countryCode);
    this.clientForm.get("phoneCountryCode").patchValue(countryCode);
  }
  upsertClient() {
  ...
  resendKycMail() {
    this.reSendKycLoadingForm = true;
    const resendKycMail = new KycConfigurationModel();
    resendKycMail.clientId = this.selectedClientId;
    this.BillingDashboardService.resendKycMail(resendKycMail)
      .subscribe((responseData: any) => {
        this.toaster.success("KYC Reshared Successfully");
        this.reSendKycLoadingForm = false;
      });
  ...
  getContractTemplatesList() {
    const templateIds = this.clientForm.value.contractTemplateId;
    if (templateIds && templateIds.length > 0) {
      const index = templateIds.indexOf(0);
      if (index > -1) {
        templateIds.splice(index, 1);
      }
      this.selectedRoleIds = templateIds;
  ...
  toggleAllTemplatesSelected() {
    if (this.templateAllSelected.selected) {
      this.clientForm.controls.contractTemplateId.patchValue([
        ...this.contractTemplatesList.map((item) => item.contractTemplateId),
        0
      ]);
  ...
  toggleTemplatePerOne(all) {
    ...
    if (
      this.clientForm.controls.contractTemplateId.value.length ===
      this.contractTemplatesList.length
    ) {
  ...
  getKycDocumentType() {
    ...
    if (this.clientForm.get("roleId").value) {
      kycConfig.roleId = this.clientForm.get("roleId").value.toString();
    }
    else if(this.selectedClientDetails && this.selectedClientDetails.roleIds) {
      kycConfig.roleId = this.selectedClientDetails.roleIds;
    }
  ...
  getClientTypeName(event) {
    if (event) {
      ...
      // if (this.selectedClientTypeName == "Ship Broker") {
      //   this.clientForm.controls["brokerageValue"].setValidators([Validators.required]);
      //   this.clientForm.get("brokerageValue").updateValueAndValidity();
      // } else if (this.selectedClientTypeName != "Ship Broker") {
      //   this.clientForm.controls["brokerageValue"].setValidators([]);
      //   this.clientForm.get("brokerageValue").updateValueAndValidity();
      //   this.clientForm.get("brokerageValue").patchValue(null);
      // }
    }
  }
  ...
  saveform() {
    ...
    if ((this.isSGTrader && !this.selectedClientDetails?.isKycSybmissionMailSent) || (!this.isSGTrader)) {
      performanceConfig.isSubmitted = true;
    }
