...
@Component({
  selector: "app-testrail-estimated-time",
  templateUrl: "./estimated-time.component.html",
  changeDetection: ChangeDetectionStrategy.OnPush
})

export class TestRailEstimatedTimeComponent implements OnInit {
  estimatedTime;
  @Input("estimatedTime")
  ...
  regexPattern: string = "^[0-9]{1,3}[w][0-9]{1,3}[d][0-9]{1,3}[h]$|[0-9]{1,3}[d][0-9]{1,3}[h]$|[0-9]{1,3}[w][0-9]{1,3}[d]$|[0-9]{1,3}[w]{1,3}[h]$|[0-9]{1,3}[d]$|[0-9]{1,3}[w]{1,3}[h]$|[0-9]{1,3}[w]|[0-9]{1,3}[d]$|[0-9]{1,3}[h]$";
  totalHours: any;
  ...
  ngOnInit() {
    if (this.router.url.toString().includes("myWork/adhoc-work")) {
      ...
      this.estimatedTimeFormControl.setValidators([Validators.required, Validators.pattern(this.regexPattern)]);
      this.cdRef.detectChanges();
    }
    else
