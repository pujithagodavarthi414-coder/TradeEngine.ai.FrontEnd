...
export class UserService {
  ...
  private GET_USER_LEVEL_ACCESS_API_PATH = APIEndpoint + "Lives/LivesApi/GetUserLevelAccess";
  private UPSERT_USER_LEVEL_ACCESS_API_PATH = APIEndpoint + "Lives/LivesApi/UpsertUserLevelAccess";
  ...
  getUserLevelAccess(model: UserLevelAccess) {
    const httpOptions = {
      headers: new HttpHeaders({ "Content-Type": "application/json" })
    };
    let body = JSON.stringify(model);
    return this.http.post(
      `${this.GET_USER_LEVEL_ACCESS_API_PATH}`,
      body,
      httpOptions
    );
  }

  upsertUserLevelAccess(model: any) {
    const httpOptions = {
      headers: new HttpHeaders({ "Content-Type": "application/json" })
    };
    let body = JSON.stringify(model);
    return this.http.post(
      `${this.UPSERT_USER_LEVEL_ACCESS_API_PATH}`,
      body,
      httpOptions
    );
  }
