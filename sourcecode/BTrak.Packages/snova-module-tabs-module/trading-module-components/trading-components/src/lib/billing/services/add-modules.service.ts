...
export class AddModulesService {
  ...
  public GetDynamicModuleTabs = APIEndpoint + 'Widgets/WidgetsApi/GetDynamicModuleTabs';
...
// }
  ...
  getGetDynamicModuleTabs(dynamicModuleUpsertModel: any) {
    const httpOptions = {
      headers: new HttpHeaders({ "Content-Type": "application/json" })
    };
    let body = JSON.stringify(dynamicModuleUpsertModel);
    return this.http.post(this.GetDynamicModuleTabs, body, httpOptions)
      .pipe(map(result => {
        return result;
      })
      );
