...
export class UserEffects {
  constructor(
    private actions$: Actions, private recruitmentService: RecruitmentService, private translateService: TranslateService) { }
  ...
  loadUsers$: Observable<Action> = this.actions$.pipe(
    ...
    switchMap(() => {
      return this.recruitmentService.GetAllUsers().pipe(
  ...
  upsertUser$: Observable<Action> = this.actions$.pipe(
    ...
    switchMap(CreateUsersTriggeredAction => {
      if (CreateUsersTriggeredAction.users.userId === null
        || CreateUsersTriggeredAction.users.userId === '' || CreateUsersTriggeredAction.users.userId === undefined) {
        this.isNewUser = true;
      } else if (
  ...
  LoggedUserId$: Observable<Action> = this.actions$.pipe(
    ...
    switchMap(searchAction => {
      return this.recruitmentService.getLoggedUserData().pipe(
