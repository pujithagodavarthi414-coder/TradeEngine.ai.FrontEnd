...
import { HeatMapTheme, ILoadedEventArgs, ITooltipEventArgs } from '@syncfusion/ej2-heatmap';
...
export class CalendarChartComponent implements AfterViewInit {
  ...
  constructor(public datePipe: DatePipe, private masterDataManagementService: MasterDataManagementService) {
  }

  getApiData() {
    const employeeListSearchResult: any = [];
    employeeListSearchResult.userId = null;
    employeeListSearchResult.SpName = this.procName;
    this.masterDataManagementService.getGenericApiData(employeeListSearchResult).subscribe((response: any) => {
  ...
  public async createChart(value): Promise<void> {
    d3.select("#" + this.dashboardId).selectAll('svg').remove();
    d3.selectAll('#week').remove();
    var week_days = ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat']
    var dateData = [];
    var legendsData = this.legendsData;
    if(this.legendsData){
      legendsData = this.legendsData.sort(this.compare);
    }
    ...
    if (this.years == 0) {
      for (var i = 0; i < 7; i++) {
        this.svg.append("text")
          ...
          .text(function () { return week_days[i]; });
        if (i != 6) {
    ...
    if (this.showData) {
      var rect = svg.selectAll("rect.day")
        .data(function (d) { return d3.timeDays(d, new Date(d.getFullYear(), d.getMonth() + 1, 1)); })
        .enter().append("g")
        .attr("class", "group")
        .append("rect")
        .attr("class", "day")
    ...
    if (this.showData) {
      rect.filter(function (d) {
        ...
        .style("fill", function (d) {
          ...
          // else {
            let color;
            // if (legendsData && legendsData.length == 2) {
            //   legendsData.forEach(element => {
            //     if (lookup[d] == parseInt(element.value)) {
            //       if (value) {
            //         if (value == element.legendColor) {
            //           color = '#eaeaea'
            //         }
            //       }
            //       else {
            //         color = element.legendColor;
            //       }
            //     }
            //     else if (lookup[d] == parseInt(legendsData[1].value)) {
            //       if (value) {
            //         if (value == element.legendColor) {
            //           color = '#eaeaea'
            //         }
             ...
             //else {
              for (var i = 0; i < legendsData.length; i++) {
                if (i != legendsData.length - 1) {
                  if (lookup[d] >= parseFloat(legendsData[i].value) && lookup[d] < parseFloat(legendsData[i + 1].value)) {
                    color = legendsData[i].legendColor;
                    if (value) {
                      if (value ==  legendsData[i].legendColor) {
                        color = '#eaeaea'
                      }
                    }
                    else {
