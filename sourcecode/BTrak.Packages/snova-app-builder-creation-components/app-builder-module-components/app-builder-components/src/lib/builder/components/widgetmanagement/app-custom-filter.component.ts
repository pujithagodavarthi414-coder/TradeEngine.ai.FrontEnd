...
import { FilterKeyValueModel } from "../../models/filter-key-valu.model";
...
import { TeamMembersListModel } from '../../models/team-member.model';
...
export class CustomAppFilterComponent extends CustomAppBaseComponent {
    ...
    selectedName: any;
    selectedStatus: any;
    selectedJobOpeningStatusId: string;
    selectedJobOpeningStatus: any;
    JobOpeningstatus: any;
   selectedHiringStatusId: string;
    selectedHiringStatus: any;
    selectedGetEmploymentStatusName: any;
    selectedEmploymentStatusName: any;
    ...
    selectedCandidateFirstName: any;
    selectedCandidateLastName: any;
    ...
    @Input("filterApplied")
    set _filterApplied(data: string) {
        if (this.filterApplied != data) {
    ...
    @Input("filtersAny")
    set _filtersAny(data: any) {
        if (data && data.isCustomApp != undefined && data.isCustomApp) {
    ...
    selectedActiveEmployee: string;
    isActiveEmployeesOnly: string = null;
    ...
    constructor(
        ...
        private roleService: RoleManagementService, private translateService: TranslateService,
        private customTagService: CustomTagService,private routes: Router
    ) {
        super();
    }
    ...
    ngOnInit() {
        ...
        this.filterAppliedToolTip = this.translateService.instant('TESTMANAGEMENT.FILTERAPPLIED');
        this.filterToolTip = this.translateService.instant('TESTMANAGEMENT.FILTER');
        this.activeList = [
            {
                name : 'All',
                value : 'all'
    ...
    getSoftLabelConfigurations() {
       ...
       if(this.canAccess_feature_CanAccessRecruitmentFilter)
       {
       this.GetSources();
    ...
    searchTestSuites() {
        ...
        .searchTestSuites(testSuiteModel)
        .subscribe((responseData: any) => {
            console.log(responseData);
            this.testSuiteList = responseData.data;
            if (this.dynamicFilters.length > 0) {
                const testSuiteIndex = this.dynamicFilters.findIndex((p) => p.filterKey === "TestSuite" && p.isSystemFilter === true);
                if (testSuiteIndex > -1 && this.testSuiteList && this.testSuiteList.length > 0) {
                    const index = this.testSuiteList.findIndex((p) => p.testSuiteId === this.dynamicFilters[testSuiteIndex].filterValue);
                    if (index > -1) {
                        this.selectedTestSuiteName = this.testSuiteList[index].testSuiteName;
                        this.cdRef.detectChanges();
                    }
                }
            }
    ...
    GetUsers() {
        this.widgetService.getTeamLeadsList().subscribe((response: any) => {
    ...
    getEntityDropDown() {
        let searchText = "";
        this.widgetService.getEntityDropDown(searchText).subscribe((responseData: any) => {
    ...
    getBranches() {
        ...
        this.widgetService.getBranches(hrBranchModel).subscribe((response: any) => {
    ...
    GetSources() {
        ...
        this.widgetService.GetSources(sourceModel).subscribe((response: any) => {
            this.Sources = response.data;
            if (this.dynamicFilters.length > 0) {
                const SourceIndex = this.dynamicFilters.findIndex((p) => p.filterKey === "Source" && p.isSystemFilter === true);
                if (SourceIndex > -1 && this.Sources && this.Sources.length > 0) {
                    const index = this.Sources.findIndex((p) => p.SourceId === this.dynamicFilters[SourceIndex].filterValue);
                    if (index > -1) {
                        this.selectedName = this.Sources[index].Name;
                        this.cdRef.detectChanges();
                    }
                }
            }
    ...
    GetCandidates() {
        ...
        //EmploymentStatusModel.isArchived = false;
        this.widgetService.GetCandidates(candidateModel).subscribe((response: any) => {
            this.Candidates = response.data;
            if (this.dynamicFilters.length > 0) {
                const CandidateIndex = this.dynamicFilters.findIndex((p) => p.filterKey === "Candidate" && p.isSystemFilter === true);
                if (CandidateIndex > -1 && this.Candidates && this.Candidates.length > 0) {
                    const index = this.Candidates.findIndex((p) => p.CandidateId === this.dynamicFilters[CandidateIndex].filterValue);
                    if (index > -1) {
                        this.selectedfirstName = this.Candidates[index].firstName;
                        this.selectedlastName = this.Candidates[index].lastName;
                        this.cdRef.detectChanges();
                    }
                }
            }
    ...
    GetJobOpeningStatus() {
        var jobOpeningStatusModel = new JobOpeningStatusModel();
        //EmploymentStatusModel.isArchived = false;
        this.widgetService.GetJobOpeningStatus(jobOpeningStatusModel).subscribe((response: any) => {
            this.JobOpeningStatus = response.data;
            if (this.dynamicFilters.length > 0) {
                const JobOpeningStatusIndex = this.dynamicFilters.findIndex((p) => p.filterKey === "JobOpeningStatus" && p.isSystemFilter === true);
                if (JobOpeningStatusIndex > -1 && this.JobOpeningStatus && this.JobOpeningStatus.length > 0) {
                    const index = this.JobOpeningStatus.findIndex((p) => p.JobOpeningStatusId === this.dynamicFilters[JobOpeningStatusIndex].filterValue);
                    if (index > -1) {
                        this.selectedStatus = this.JobOpeningStatus[index].Status;
                        this.cdRef.detectChanges();
                    }
                }
            }
    ...
    GetEmploymentStatus() {
        var employmentStatusModel = new EmploymentStatusModel();
        //EmploymentStatusModel.isArchived = false;
        this.widgetService.GetEmploymentStatus(employmentStatusModel).subscribe((response: any) => {
            this.EmploymentStatus = response.data;
            if (this.dynamicFilters.length > 0) {
                const EmploymentStatusIndex = this.dynamicFilters.findIndex((p) => p.filterKey === "EmploymentStatus" && p.isSystemFilter === true);
                if (EmploymentStatusIndex > -1 && this.EmploymentStatus && this.EmploymentStatus.length > 0) {
                    const index = this.EmploymentStatus.findIndex((p) => p.EmploymentStatusId === this.dynamicFilters[EmploymentStatusIndex].filterValue);
                    if (index > -1) {
                        this.selectedEmploymentStatus = this.EmploymentStatus[index].employmentStatusName ;
                        this.cdRef.detectChanges();
                    }
                }
            }
    ...
    GetHiringStatus() {
        var hiringStatusModel = new HiringStatusModel();
        //EmploymentStatusModel.isArchived = false;
        this.widgetService.GetHiringStatus(hiringStatusModel).subscribe((response: any) => {
            this.HiringStatus = response.data;
            if (this.dynamicFilters.length > 0) {
                const HiringStatusIndex = this.dynamicFilters.findIndex((p) => p.filterKey === "HiringStatus" && p.isSystemFilter === true);
                if (HiringStatusIndex > -1 && this.HiringStatus && this.HiringStatus.length > 0) {
                    const index = this.HiringStatus.findIndex((p) => p.HiringStatusId === this.dynamicFilters[HiringStatusIndex].filterValue);
                    if (index > -1) {
                        this.selectedStatus = this.HiringStatus[index].Status;
                        this.cdRef.detectChanges();
                    }
                }
            }
    ...
    getFilterKeys(isEmitRequired) {
        ...
        this.widgetService.GetCustomDashboardFilters(dashboardDynamicFilters).subscribe((result: any) => {
            if (result.success === true) {
                ...
                this.activeFiltersCount = this.dynamicFilters.filter(p => p.filterValue != null && p.filterValue != "").length;
                this.cdRef.detectChanges();
    ...
    searchCustomTags(isEmiteRequired) {
        ...
        const testSuiteTag = new FilterKeyValueModel();
        testSuiteTag.filterKey = "TestSuite";
        testSuiteTag.filterName = "TestSuite";
        testSuiteTag.isSystemFilter = true;
        const EmploymentStatusTag = new FilterKeyValueModel();
        EmploymentStatusTag.filterKey = "EmploymentStatus";
        EmploymentStatusTag.filterName = "EmploymentStatus";
        EmploymentStatusTag.isSystemFilter = true;
        const SourceTag = new FilterKeyValueModel();
        SourceTag.filterKey = "Source";
        SourceTag.filterName = "Source";
        SourceTag.isSystemFilter = true;
        const CandidateTag = new FilterKeyValueModel();
        CandidateTag.filterKey = "Candidate";
        CandidateTag.filterName = "Candidate";
        CandidateTag.isSystemFilter = true;
        const JobOpeningStatusTag = new FilterKeyValueModel();
        JobOpeningStatusTag.filterKey = "JobOpeningStatus";
        JobOpeningStatusTag.filterName = "JobOpeningStatus";
        JobOpeningStatusTag.isSystemFilter = true;
        const HiringStatusTag = new FilterKeyValueModel();
        HiringStatusTag.filterKey = "HiringStatus";
        HiringStatusTag.filterName = "HiringStatus";
        HiringStatusTag.isSystemFilter = true;
        ...
        if (this.dynamicFilters.length > 0) {
            ...
            if (testSuiteIndex > -1 && this.testSuiteList && this.testSuiteList.length > 0) {
                ...
                if (index > -1) {
                    this.selectedTestSuiteName = this.testSuiteList[index].testSuiteName;
                    testSuiteTag.filterId = this.dynamicFilters[testSuiteIndex].filterId;
                    testSuiteTag.filterValue = this.dynamicFilters[testSuiteIndex].filterValue;
                    this.selectedTestSuiteId = this.dynamicFilters[testSuiteIndex].filterValue;
                    this.cdRef.detectChanges();
                } else {
                    this.selectedTestSuiteName = null;
                    this.selectedTestSuiteId = null;
                }
            }
            ...
            if (dateIndex > -1) {
                ...
                if (this.date == 'lastWeek' || this.date == 'nextWeek' || this.date == 'thisMonth' || this.date == 'lastMonth') {
                    ...
                    //       this.dynamicFilters[dateIndex].filterValue = "{\"dateFrom\":\"2021-01-01T01:30:00.000Z\",\"dateTo\":\""+this.curDate+"\"}";
                    //       this.activeFiltersCount = this.dynamicFilters.filter(p => p.filterValue != null && p.filterValue != "").length;
                    //      }
                    //   this.date ="{\"dateFrom\":\"2021-01-01T01:30:00.000Z\",\"dateTo\":\""+this.curDate+"\"}";
                    //   let obj = JSON.parse(this.date);
                    //   this.dateFrom = obj.dateFrom;
                    //   this.tempDateFrom = obj.dateFrom;
                    //   this.dateTo = obj.dateTo;
        ...
        this.customTags.push(testSuiteTag);
        this.customTags.push(EmploymentStatusTag);
        this.customTags.push(SourceTag);
        this.customTags.push(CandidateTag);
        this.customTags.push(JobOpeningStatusTag);
        this.customTags.push(HiringStatusTag);
        // if(this.isDashboard){
    ...
    onChangeTag(selectedValue) {
        ...
        }else  if (selectedValue.filterKey === "TestSuite" && selectedValue.isSystemFilter === true) {
        ...
        else if (selectedValue.filterKey === "EmploymentStatus" && selectedValue.isSystemFilter === true) {
            this.selectedFilterValue = "17";
            this.selectedDynamicFilter = selectedValue;
        }
        else if (selectedValue.filterKey === "Source" && selectedValue.isSystemFilter === true) {
            this.selectedFilterValue = "18";
            this.selectedDynamicFilter = selectedValue;
        }
        else if (selectedValue.filterKey === "Candidate" && selectedValue.isSystemFilter === true) {
            this.selectedFilterValue = "20";
            this.selectedDynamicFilter = selectedValue;
        }
        else if (selectedValue.filterKey === "JobOpeningStatus" && selectedValue.isSystemFilter === true) {
            this.selectedFilterValue = "19";
            this.selectedDynamicFilter = selectedValue;
        }
        else if (selectedValue.filterKey === "HiringStatus" && selectedValue.isSystemFilter === true) {
            this.selectedFilterValue = "16";
            this.selectedDynamicFilter = selectedValue;
        }
    }
    ...
    activeEmployeeSelected(event) {
        if (this.isActiveEmployeesOnly == 'active') {
            this.selectedActiveEmployee = "true";
        } else if(this.isActiveEmployeesOnly == 'inactive'){
            this.selectedActiveEmployee = "false";
        } else {
            this.selectedActiveEmployee = "all";
        }
    }
    ...
    JobOpeningStatusSelected(id){
        this.selectedJobOpeningStatusId = id;
    }

    EmploymentStatusSelected(id){
        this.selectedEmploymentStatusId = id;
    }

    HiringStatusSelected(id){
        this.selectedHiringStatusId = id;
    }
    ...
    applyDashboardFilters() {
        ...
        if (this.selectedFilterValue === "14" && this.selectedTestSuiteId === null) {
            this.filterValueRequired = true;
        } else if (this.selectedFilterValue === "14") {
            const index = this.testSuiteList.findIndex((p) => p.testSuiteId === this.selectedTestSuiteId);
            if (index > -1) {
                this.selectedTestSuiteName = this.testSuiteList[index].testSuiteName;
                this.selectedDynamicFilter.isSystemFilter = true;
                this.selectedDynamicFilter.filterValue = this.selectedTestSuiteId;
            } else {
                this.selectedTestSuiteName = null;
            }
        }
        ...
        if (this.selectedFilterValue === "15" && this.selectedBusinessUnitId === null) {
            this.filterValueRequired = true;
        } else if (this.selectedFilterValue === "15") {
        ...
        if (this.selectedFilterValue === "18" && this.selectedSourceId === null) {
            this.filterValueRequired = true;
        } else if (this.selectedFilterValue === "18") {
            const SourceIndex = this.Sources.findIndex((p) => p.sourceId === this.selectedSourceId);
            if (SourceIndex > -1) {
                this.selectedSourceName = this.Sources[SourceIndex].name;
                this.selectedSource = this.selectedSourceName;
                this.selectedDynamicFilter.isSystemFilter = true;
                this.selectedDynamicFilter.filterValue = this.selectedSourceId;
            } else {
        ...
        if (this.selectedFilterValue === "20" && this.selectedCandidateId === null) {
            this.filterValueRequired = true;
        } else if (this.selectedFilterValue === "20") {
            const  CandidateIndex = this.Candidates.findIndex((p) => p.candidateId === this.selectedCandidateId);
            if ( CandidateIndex > -1) {
                this.selectedCandidateFirstName = this.Candidates[CandidateIndex].firstName;
                this.selectedCandidateLastName = this.Candidates[CandidateIndex].lastName;
                this.selectedCandidate = this.selectedCandidateFirstName + " "+this.selectedCandidateLastName;
                this.selectedDynamicFilter.isSystemFilter = true;
                this.selectedDynamicFilter.filterValue = this.selectedCandidateId;
            } else {
                this.selectedCandidateFirstName = null;
                this.selectedCandidateLastName = null;
                this.selectedCandidate = null;
            }
        }

        if (this.selectedFilterValue === "19" && this.selectedJobOpeningStatusId === null) {
            this.filterValueRequired = true;
        } else if (this.selectedFilterValue === "19") {
            const JobOpeningStatusIndex = this.JobOpeningStatus.findIndex((p) => p.jobOpeningStatusId === this.selectedJobOpeningStatusId);
            if (JobOpeningStatusIndex > -1) {
                this.selectedJobOpeningStatus = this.JobOpeningStatus[JobOpeningStatusIndex].status;
                this.selectedDynamicFilter.isSystemFilter = true;
                this.selectedDynamicFilter.filterValue = this.selectedJobOpeningStatusId;
            } else {
                this.selectedJobOpeningStatus = null;
            }
        }

        if (this.selectedFilterValue === "17" && this.selectedEmploymentStatusId === null) {
            this.filterValueRequired = true;
        } else if (this.selectedFilterValue === "17") {
            const EmploymentStatusIndex = this.EmploymentStatus.findIndex((p) => p.employmentStatusId === this.selectedEmploymentStatusId);
            if (EmploymentStatusIndex > -1) {
                this.selectedEmploymentStatusName = this.EmploymentStatus[EmploymentStatusIndex].employmentStatusName;
                this.selectedEmploymentStatus = this.selectedEmploymentStatusName;
                this.selectedDynamicFilter.isSystemFilter = true;
                this.selectedDynamicFilter.filterValue = this.selectedEmploymentStatusId;
            } else {
                this.selectedEmploymentStatusName = null;
        ...
        if (this.selectedFilterValue === "16" && this.selectedHiringStatusId === null) {
            this.filterValueRequired = true;
        } else if (this.selectedFilterValue === "16") {
            const HiringStatusIndex = this.HiringStatus.findIndex((p) => p.hiringStatusId === this.selectedHiringStatusId);
            if (HiringStatusIndex > -1) {
                this.selectedHiringStatus = this.HiringStatus[HiringStatusIndex].status;
                this.selectedDynamicFilter.isSystemFilter = true;
                this.selectedDynamicFilter.filterValue = this.selectedHiringStatusId;
            } else {
                this.selectedHiringStatus  = null;
        ...
        if (this.selectedFilterValue === "10" && this.selectedActiveEmployee === null) {
            ...
            if(this.selectedActiveEmployee == "all") {
                this.selectedActiveEmployee = null;
            }
            this.selectedDynamicFilter.isSystemFilter = true;
    ...
    upsertDynamicFilters() {
        const dashboardDynamicFilters = new DynamicDashboardFilterModel();
        dashboardDynamicFilters.referenceId = this.selectedAppId == null ? this.selectedWorkspaceId : this.selectedAppId;
        ...
        this.widgetService.UpsertCustomDashboardFilter(dashboardDynamicFilters).subscribe((result: any) => {
            if (result.success === true) {
                ...
                this.dashboardFilter.testSuiteId = this.selectedTestSuiteId;
                this.dashboardFilter.auditId = this.selectedAuditId;
                this.dashboardFilter.BusinessUnitIds = this.selectedBusinessUnitId;
    ...
    ngOnChanges(): void {
        ...
        if (this.dashboardGlobalData && this.dashboardGlobalData.testSuiteList) {
            this.testSuiteList = this.dashboardGlobalData.testSuiteList;
        }
        ...
        if (this.dashboardGlobalData && this.dashboardGlobalData.SourceList) {
            this.Sources = this.dashboardGlobalData.SourceList;
        }

        if (this.dashboardGlobalData && this.dashboardGlobalData.CandidateList) {
            this.Candidates = this.dashboardGlobalData.CandidateList;
        }

        if (this.dashboardGlobalData && this.dashboardGlobalData.JobOpeningStatusList) {
            this.JobOpeningstatus = this.dashboardGlobalData.JobOpeningStatusList;
        }

        if (this.dashboardGlobalData && this.dashboardGlobalData.EmploymentStatusList) {
            this.EmploymentStatus= this.dashboardGlobalData.EmploymentStatusList;
        }

        if (this.dashboardGlobalData && this.dashboardGlobalData.HiringStatusList) {
            this.HiringStatus = this.dashboardGlobalData.HiringStatusList;
        }
