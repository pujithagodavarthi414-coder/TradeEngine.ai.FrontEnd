import { Component, OnInit, Input, ChangeDetectorRef, Inject, ViewChild } from '@angular/core';
...
export class WorkFlowTriggerDialogComponent implements OnInit {
    ...
    set _data(data: any) {
        if (data && data !== undefined) {
            this.workflowEdit = data[0].workflowEdit;
    ...
    constructor(private genericStatusSerive: GenericStatusService, private toastr: ToastrService, public dialogRef: MatDialogRef<WorkFlowTriggerDialogComponent>, @Inject(MAT_DIALOG_DATA) private data: any, public dialog: MatDialog, private cdRef: ChangeDetectorRef) {
    ...
    getTriggers() {
        ...
        this.genericStatusSerive.getTriggers(triggerModel).subscribe((responseData: any) => {
    ...
    getWorkflows() {
        let triggerModel = new WorkflowTrigger();
        triggerModel.isArchived = false;
        this.genericStatusSerive.getWorkflowsForTriggers(triggerModel).subscribe((responseData: any) => {
            let success = responseData.success;
            if (success) {
    ...
    upsertWorkflowTrigger(workflowId) {
        ...
        this.modeler.saveXML((err: any, xml: any) => {
            ...
            this.genericStatusSerive.upsertWorkflowTrigger(triggerModel).subscribe((responseData: any) => {
                let success = responseData.success;
                if (success) {
    ...
    this.genericStatusSerive.UploadFile(file).subscribe((responseData: any) => {
