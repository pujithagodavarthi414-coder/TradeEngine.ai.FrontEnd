...
import { DashboardOrderModel } from '../models/models/dashboardOrderModel';
...
export class WidgetService {
	...
	addWidgetToFavourites(widget) {
		...
		return this._http.post(APIEndpoint + "Widgets/WidgetsApi/AddWidgetToFavourites",body, httpOptions)
		.pipe(map(result => {
			return result;
		}));
	...
	insertRecentSearch(search: RecentSearchApiModel) {
		...
		return this._http.post(APIEndpoint + "RecentSearch/RecentSearchApi/InsertRecentSearch",body, httpOptions)
			.pipe(map(result => {
				return result;
			}));
	  }

	  getRecentSearchedApps(search: WidgetList) {
		let environment = JSON.parse(localStorage.getItem(LocalStorageProperties.Environment));
		let APIEndpoint = environment.apiURL;
		const httpOptions = {
			headers: new HttpHeaders({ 'Content-Type': 'application/json' })
		...
		let body = JSON.stringify(search);
		return this._http.post<any[]>(APIEndpoint + "RecentSearch/RecentSearchApi/GetRecentSearchedApps", body, httpOptions)
		  .pipe(map(result => {
			return result;
		  }));
	  }

	upsertTag(TagIdList: any) {
		const httpOptions = {
		  headers: new HttpHeaders({ 'Content-Type': 'application/json' })
		};
	...
	updateDashboard(dashboard: DashboardList) {
		...
		var dashboardClone = {...dashboard};

		_.each(dashboardClone.dashboard, function(widget){
	...
	GetWidgetsBasedOnUserId(widget: WidgetList) {
		const httpOptions = {
			headers: new HttpHeaders({ "Content-Type": "application/json" })
		};
	...
	getAddOrEditCustomAppIsRequired() {
		...
		return this._http.get(APIEndpoint + ApiUrls.GetAddOrEditCustomAppIsRequired, httpOptions)
		  .pipe(map((result) => {
			return result;
		  }));
	  }
