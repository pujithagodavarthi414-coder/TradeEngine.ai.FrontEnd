...
@Component({
    ...
    template: "",
    viewProviders: [TimeUsageService]
})
export class ActivityTrackerBryntumSchedulerComponent implements OnInit, OnChanges, OnDestroy {
    ...
    @Input() groupFeature: boolean | object | string = "name";
    ...
    ngOnChanges(changes: SimpleChanges) {
        ...
        if (me.schedulerEngine) {
            ...
            if (changes && (changes._groupType)) {
                this.schedulerEngine.features["group"] = this.groupFeature;
                this.schedulerEngine.config["columns"] = this.columns;
            }
            this.createEmployeeAsResource();
            this.loadGridData();
            ...
            template: data => {
                ...
                if (data.eventRecord && data.eventRecord.data) {
                    ...
                    if (event.applicationTypeName == "Productive") {
                        ...
                        name = "Break";
                    } else if (event.applicationTypeName == "Leave") {
                        name = "Leave";
    ...
    loadGridData() {
        if (this.trackerData && this.trackerData.length > 0) {
            this.events = this.trackerData.map((event, index) => {
                let startTime;
                let endTime;
                if (event.applicationTypeName == "Log" || event.applicationTypeName == "Lunch" || event.applicationTypeName == "Break") {
