...
export class ActivityTrackerBryntumReportView extends CustomAppBaseComponent implements OnInit {
    ...
    set _branchData(data: any) {
      if(data) {
        ...
        this.allBranchIds = this.branchesList.map(x => x.branchId);
    ...
    set _departmentData(data: any) {
      if(data) {
        this.departmentList = data;
        this.alldepartmentIds = this.departmentList.map(x => x.departmentId);
        this.setCreatePlanForm();
        this.initiateReport();
      }
    }
    ...
    idleTimeOverlayChecker: boolean = false;
    idleTimeData: any;
    ...
    ngAfterViewInit() {
        this.introJS.setOptions({
            steps: [
                {
                    element: '#timeline-1',
                    intro: "It will diplay all the timeline details which are tracked.",
                    position: 'bottom'
                },
                {
                    element: '#timeline-2',
                    intro: "If we select this Activity Tracker checkbox, it will shows the total tracker time and neutral time with progress bar.",
                    position: 'bottom'
                },
                {
                    element: '#timeline-3',
                    intro: "If we select this Timesheet Overlay checkbox, it will display all the timing reports of the tracked user.",
                    position: 'bottom'
                },
                {
                    element: '#timeline-4',
                    intro: "If we select this User Story Overlay checkbox, it will display story timeline reports of the tracked user.",
                    position: 'bottom'
                },
                {
                    element: '#timeline-5',
                    intro: "If we select this Idle Time Overlay checkbox, it will display idle time of the tracked user.",
                    position: 'bottom'
                },
                {
                    element: '#timeline-6',
                    intro: "It is used for filtering the data.",
                    position: 'bottom'
                }
    ...
    constructor(private formBuilder: FormBuilder, private timeUsageService: TimeUsageService,
        private cdRef: ChangeDetectorRef, private activatedRoute: ActivatedRoute, private toastr: ToastrService, private cookieService: CookieService,
        ...
        this.timeUsageService.getEmployeeDetails(employeeListSearchResult).subscribe((response: any) => {
            if (response.success) {
                if (this.trackerFilterJson && this.trackerFilterObject && this.trackerFilterObject.initialFilter) {
                    ...
                    this.allEmployeeIds = this.employeeListDataDetails.map(p => p.userId);
    ...
    drawReport() {
        if (this.trackerFilterJson) {
            ...
            if (this.departmentList && this.branchesList) {
                ...
                employeeListSearchResult.pageNumber = this.page.pageNumber + 1;
                employeeListSearchResult.pageSize = this.page.size;
                employeeListSearchResult.isReporting = true;
                employeeListSearchResult.isArchived = false;
                if (this.canAccess_feature_ViewActivityReportsForAllEmployee) {
                    employeeListSearchResult.isPermission = true;
                }
                else {
                    employeeListSearchResult.isPermission = false;
                }
                this.timeUsageService.getEmployeeDetails(employeeListSearchResult).subscribe((response: any) => {
                    if (response.success) {
                        this.employeeListDataDetails = response.data.employeePersonalDetails;
    ...
    introStart() {
        ...
        this.introJS.setOption('doneLabel', 'Next page').start().oncomplete(() => {
            if (this.canAccess_feature_ViewActivityScreenshots) {
