...
export class LiveDashboardService {
  ...
  getAllProcessDashboardDetails(statusColor: string,entityId): Observable<LiveDashBoardList[]> {
    let paramsobj = new HttpParams().set('statusColor', statusColor).set("entityId",entityId);
    const httpOptions = {
      headers: new HttpHeaders({ 'Content-Type': 'application/json', 'Authorization': 'my-auth-token' }),
      params: paramsobj
    };
