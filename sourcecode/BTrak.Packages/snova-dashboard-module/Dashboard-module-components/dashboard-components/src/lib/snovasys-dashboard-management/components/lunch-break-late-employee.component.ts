...
@Component({
  selector: 'app-dashboard-component-lunchBreakLateEmployee',
  template: `

  <!-- Lunch Break Late Employees-->
  <div class="full-height" *ngIf="(canAccess_feature_LateEmployee)">
      ...
      <mat-card-title class="full-width data-table-header drag-handler">
        <div fxLayout="row wrap" fxLayoutGap="0" fxLayoutAlign="end center">
          <div fxFlex class="card-title-text data-table-header p-05">
            {{'HRDASHBOARD.LUNCHBREAKLATEEMPLOYEE' | translate | softLabelsPipe : softLabels | titlecase}}</div>
    ...
    <div class="activity-data ">
      <mat-card-content class="p-0 m-0">
        <div class="activity-data">
          <kendo-grid [data]="gridData" [scrollable]="'none'" [pageSize]="state.take" [skip]="state.skip"
            ...
            <kendo-grid-column field="daysLate" title="{{'HRDASHBOARD.DAYSLATE' | translate}}"></kendo-grid-column>
          </kendo-grid>
...
export class LunchBreakLateEmployeeComponent extends CustomAppBaseComponent implements OnInit {
  ...
  lunchBreakLateEmployeeData: any;
  ...
  monthweekfilteractive: boolean;
  selectBranchFilterIsActive: boolean = false;
  monthweekfil

  teractive: boolean = true;
  ...
  constructor(private hrDashboardService: HrDashboardService,
    ...
    super();
    this.getLunchBreakLongTimeTakeEmployeeDetails();
    this.getSoftLabels();
  }
  ...
  dataStateChange(state: DataStateChangeEvent): void {
    this.state = state;
    this.gridData = process(this.lunchBreakLateEmployeeData, this.state);
  }

  getLunchBreakLongTimeTakeEmployeeDetails() {
    ...
    hrDashboardModel.IsLunchBreakLongTake = true;
    ...
    if (this.selectedDate && this.type) {
      this.hrDashboardService.getLateEmployeeDetails(hrDashboardModel).subscribe((responseData: any) => {
        this.lunchBreakLateEmployeeData = responseData.data;
        this.gridData = process(this.lunchBreakLateEmployeeData, this.state);
