...
export class ExitWorItemDialogComponent extends CustomAppBaseComponent {
    ...
    updateExitConfig(config) {
        ...
        exitModel.userId = this.loggedInUserId;
        console.log(exitModel);
        this.dashboardService.upsertExitConfiguration(exitModel).subscribe((response: any) => {
            if (response.success == true) {
            }
        });
    }

    // updateExitConfig(exitId,isShow,exitName) {
    //     const exitModel = new ExitModel();
    //     exitModel.exitId = exitId;
    //     exitModel.isShow = isShow;
    //     exitModel.exitName = exitName;
    //     exitModel.isArchived = false;
    //     this.dashboardService.upsertExitConfiguration(exitId,exitName).subscribe((response: any) => {
    //         if (response.success == true) {
    //         }
    //     });
    // }
        ...
        if (this.isValid) {
            this.userStoriesList.forEach((userStoryDetails) => {
                if (userStoryDetails.isShow) {
                    ...
                    console.log(userStory);
                   // this.adhocWorkService.upsertAdhocWork().subscribe((result: any) => {
