...
export class MyProfileService {
  ...
  getHistoricalCompletedWork(HistoricalWorkReportModel: HistoricalWorkReportModel) {
    const httpOptions = {
      headers: new HttpHeaders({ 'Content-Type': 'application/json' }),
    };
    let body = JSON.stringify(HistoricalWorkReportModel);
    return this.http.post(APIEndpoint + ApiUrls.GetUserHistoricalCompletedWorkReport, body, httpOptions)
      .pipe(map(result => {
        return result;
      }));
  }
  downloadHistoricalWork(historicalWorkReportModel: HistoricalWorkReportModel) {
    const httpOptions = {
      headers: new HttpHeaders({ 'Content-Type': 'application/json' }),
    };
    let body = JSON.stringify(historicalWorkReportModel);
    return this.http.post(APIEndpoint + ApiUrls.GetWorkReportDetailsUploadTemplate, body, httpOptions)
      .pipe(map(result => {
        return result;
      }));
  }
  downloadHistoricalCompletedWork(historicalWorkReportModel: HistoricalWorkReportModel) {
    const httpOptions = {
      headers: new HttpHeaders({ 'Content-Type': 'application/json' }),
    };
    let body = JSON.stringify(historicalWorkReportModel);
    return this.http.post(APIEndpoint + ApiUrls.GetCompletedWorkReportDetailsUploadTemplate, body, httpOptions)
      .pipe(map(result => {
        return result;
      }));
  }
