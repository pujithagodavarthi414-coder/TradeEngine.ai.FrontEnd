...
@Component({
  selector: 'app-Workrealated-component',
  templateUrl: 'completed-workitem-component.html',
})

export class CompletedWorkItemsComponent extends CustomAppBaseComponent implements OnInit {
  ...
  getHistoricalWork() {
    ...
    this.profileService.getHistoricalCompletedWork(historicalWorkReportModel).subscribe((response: any) => {
      if (response.success == true) {
        this.historicalWork = {
          data: response.data,
  ...
  downloadFile(){
    ...
    this.profileService.downloadHistoricalCompletedWork(historicalWorkReportModel).subscribe((responseData: any) => {
