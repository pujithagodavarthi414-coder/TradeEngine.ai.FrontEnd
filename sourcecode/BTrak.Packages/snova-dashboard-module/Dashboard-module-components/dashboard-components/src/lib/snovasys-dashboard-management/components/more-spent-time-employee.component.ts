...
@Component({
  ...
  selector: 'app-dashboard-component-moreSpentTimeEmployee',
  template: `    
  <!-- More Spent Time Employees-->
    ...
    <mat-card class="p-0 m-0">
      <mat-card-title class="full-width data-table-header drag-handler">
        <div fxLayout="row wrap" fxLayoutGap="0" fxLayoutAlign="end center">
          <div fxFlex class="card-title-text data-table-header p-05">
            {{'HRDASHBOARD.MORESPENTTIMEEMPLOYEE' | translate  | softLabelsPipe : softLabels | titlecase}}</div>
      ...
      <mat-card-content class="p-0 m-0">
        <div class="activity-data">
          <kendo-grid [data]="gridData" [scrollable]="true" [pageSize]="state.take" [skip]="state.skip"
            ...
            <kendo-grid-column width="200" field="spentTime" title="{{'HRDASHBOARD.SPENTBY' | translate}}"></kendo-grid-column>
          </kendo-grid>
...
export class MoreSpentTimeEmployeeComponent extends CustomAppBaseComponent implements OnInit {
  ...
  weekNumber: number;
  direction: any;
  moreSpentTimeEmployeeData: any;
  anyOperationInProgress: boolean;
  isPermissionForMoreSpentTimeEmployee: Boolean;
  ...
  constructor(
    ...
    super();
    this.getMoreSpentTimeEmployeeDetails();
  }
  ...
  ngOnInit() {
    ...
    this.state.skip = 0;
    this.state.take = 10;
    this.isPermissionForMoreSpentTimeEmployee = this.canAccess_feature_LateEmployee;
    this.getSoftLabels();
    //this.getLateEmployee();
    if (this.isPermissionForMoreSpentTimeEmployee) {
      this.getMoreSpentTimeEmployeeDetails();
  ...
  dataStateChange(state: DataStateChangeEvent): void {
    this.state = state;
    //this.gridData = process(this.moreSpentTimeEmployeeData, this.state);
  ...
  getMoreSpentTimeEmployeeDetails() {
    this.anyOperationInProgress = true;
    var hrDashboardModel = new HrDashboardModel;
    hrDashboardModel.type = this.type;
    hrDashboardModel.Date = this.datePipe.transform(this.selectedDate, 'yyyy-MM-dd');
    hrDashboardModel.IsMoreSpentTime = true;
    hrDashboardModel.sortDirectionAsc =  this.sortDirection;
    hrDashboardModel.sortBy = this.sortBy;
    hrDashboardModel.state = this.state;
    ...
    if (this.selectedDate && this.type) {
      this.hrDashboardService.getLateEmployeeDetails(hrDashboardModel).subscribe((responseData: any) => {
        this.moreSpentTimeEmployeeData = responseData.data;
        this.gridData = {
