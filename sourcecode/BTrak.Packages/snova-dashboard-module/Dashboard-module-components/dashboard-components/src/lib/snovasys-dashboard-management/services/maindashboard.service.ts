...
export class MainDashboardService {
  ...
  getActivelyRunningProjectGoal(entityId: string, projectId: string) {
    let paramobj = new HttpParams().set("entityId",entityId).set("projectId", projectId);
    const httpOptions = {
  ...
  getActivelyRunningTeamLeadGoals(entityId,projectId) {
    let paramObj=new HttpParams().set("entityId",entityId).set("projectId", projectId);
    const httpOptions = {
      headers: new HttpHeaders({ 'Content-Type': 'application/json' }),
      params:paramObj
