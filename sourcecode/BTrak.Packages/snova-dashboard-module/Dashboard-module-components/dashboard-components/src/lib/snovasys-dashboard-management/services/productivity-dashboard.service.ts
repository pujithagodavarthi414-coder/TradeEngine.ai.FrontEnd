...
export class ProductivityDashboardService {
  ...
  private EMPLOYEE_INDEX_DRILLDOWN_API_PATH = APIEndpoint + ApiUrls.GetProduvtivityIndexDrillDownExcelTemplate;
  ...
  GetProduvtivityIndexDrillDownExcelTemplate(productivityDashboard: ProductivityDashboardModel) {
    let body = JSON.stringify(productivityDashboard);
    const httpOptions = {
      headers: new HttpHeaders({ 'Content-Type': 'application/json', 'Authorization': 'my-auth-token' }),
    };
    return this.http.post<any[]>(`${this.EMPLOYEE_INDEX_DRILLDOWN_API_PATH}`, body, httpOptions)
      .pipe(map(result => {
        return result;
      }));
  }
