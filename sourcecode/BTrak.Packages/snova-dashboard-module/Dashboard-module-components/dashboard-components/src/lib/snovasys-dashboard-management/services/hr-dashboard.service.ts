...
GetEmployeeSpentTime(userId,fromDate,toDate,entityId){
  ...
  GetLateEmployeeCount(canteenBalanceSearchInputModel: CanteenBalanceSearchInputModel) {
    const httpOptions = {
     headers: new HttpHeaders({ 'Content-Type': 'application/json' }),
    };
    let body = JSON.stringify(canteenBalanceSearchInputModel);
    return this.http.post(APIEndpoint + ApiUrls.GetLateEmployeeCount, body, httpOptions)
     .pipe(map(result => {
      return result;
     }));
    }
