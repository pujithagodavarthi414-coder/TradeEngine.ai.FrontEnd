...
export class StatusreportService {
  ...
  UpsertProbationConfiguration(performanceConfigurationModel: PerformanceConfigurationModel) {
    const httpOptions = {
      headers: new HttpHeaders({ 'Content-Type': 'application/json' })
    };
    ...
    return this.http.post(APIEndpoint + ApiUrls.UpsertProbationConfiguration, body, httpOptions)
      .pipe(map((result) => {
        return result;
      }));
  }
