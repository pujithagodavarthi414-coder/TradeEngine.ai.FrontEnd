...
import {
    ...
    WorkspaceException, WorkspaceFailed, WorkspacesActionTypes, LoadWorkspacesCompletedIfNoId, LoadChildWorkspacesTriggered, LoadChildWorkspacesCompleted
...
export class WorkspaceEffects {
            ...
            return this.WidgetService.GetWorkspaceList(getAction.WorkspacesList).pipe(
                map((Workspaces: any) => {
                    if (Workspaces.success == true) {
                        localStorage.setItem(LocalStorageProperties.Dashboards, JSON.stringify(Workspaces.data));
    ...
    @Effect()
    loadChildWorkspaces$: Observable<Action> = this.actions$.pipe(
        ofType<LoadChildWorkspacesTriggered>(WorkspacesActionTypes.LoadChildWorkspacesTriggered),
        switchMap(getAction => {
            return this.WidgetService.UpsertChildWorkspace(getAction.workspace).pipe(
                map((workspaces: any) => {
                    if (workspaces.success == true) {
                        this.workspaceId = workspaces.data;
                        this.newWorkspace = true;
                        this.hideWorkspace = false;
                        this.snackBarMessage = getAction.workspace.workspaceName + " " + this.translateService.instant("APP.DASHBOARDCREATEDSUCCESSFULLY");
                        return new LoadChildWorkspacesCompleted(workspaces.data);
                    } else {
                        this.validationMessages = workspaces.apiResponseMessages
                        return new WorkspaceFailed(workspaces.apiResponseMessages);
                    }
    ...
    @Effect()
    loadChildWorkspacesSuccessful$: Observable<Action> = this.actions$.pipe(
        ofType<LoadChildWorkspacesCompleted>(WorkspacesActionTypes.LoadChildWorkspacesCompleted),
        pipe(
            map(
                () => {
                    this.searchWorkspace = new WorkspaceList();
    ...
    loadHiddenWorkspacesListTriggered$: Observable<Action> = this.actions$.pipe(
        ...
        switchMap(getAction => {
            return this.WidgetService.GetWorkspaceList(getAction.WorkspacesList).pipe(
                map((Workspaces: any) => {
                    if (Workspaces.success == true) {
                        localStorage.setItem(LocalStorageProperties.AllDashboards, JSON.stringify(Workspaces.data));
