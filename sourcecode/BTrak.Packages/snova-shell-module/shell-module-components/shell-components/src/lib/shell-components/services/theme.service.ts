...
import * as domHelper from "../../globaldependencies/helpers/dom.helper";
...
export class ThemeService {
  ...
  changeTheme(theme) {
    ...
    var currentTheme = filteredThemes[0] != undefined || filteredThemes[0] != null ? filteredThemes[0] : {
      name: 'egret-blue',
      baseColor: '#247ba0',
      isActive: false,
      companyThemeId: 'FD2ABC0F-F8D7-47C4-A7BD-BAA637E14BD5'};
    localStorage.setItem('themeColor', currentTheme.baseColor)
    domHelper.changeTheme(this.egretThemes, currentTheme.name);
    this.egretThemes.forEach((t) => {
      t.isActive = false;
      if (t.name === currentTheme.name) {
        t.isActive = true;
        this.activatedThemeName = currentTheme.name;
      }
    });
  }
}
