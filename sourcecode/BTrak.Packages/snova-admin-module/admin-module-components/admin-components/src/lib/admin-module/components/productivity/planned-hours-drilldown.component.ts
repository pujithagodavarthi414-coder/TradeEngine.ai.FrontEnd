...
@Component({
  selector: 'lib-planned-hours-drilldown',
  templateUrl: './planned-hours-drilldown.component.html',
  styleUrls: ['./planned-hours-drilldown.component.css']
})
export class PlannedHoursDrilldownComponent implements OnInit {

  plannedHoursDrilldownsData: GridDataResult = {
    data: [],
    total: 0
  };
  state: State = {
  ...
  constructor(public dialogRef: MatDialogRef<PlannedHoursDrilldownComponent>, private productivityService: ProductivityService,
  ...
  getPlannedHoursDrillDown() {
    ...
    this.productivityService.getPlannedDetailsDrillDown(getProductivityDetailsInputModel).subscribe((res: any) => {
      if (res.success === true) {
        if (res.apiResponseMessages.length == 0) {
          this.temp = res.data;
          this.filteredList = res.data;
          if (this.temp != null) {
            this.plannedHoursDrilldownsData = {
