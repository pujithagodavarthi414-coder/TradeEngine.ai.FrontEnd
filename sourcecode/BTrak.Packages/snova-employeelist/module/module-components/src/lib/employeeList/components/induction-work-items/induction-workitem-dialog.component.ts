...
export class InductionWorItemDialogComponent extends CustomAppBaseComponent {
    ...
    ngOnInit() {
        ...
        this.employeeService.getUsersDropDown('').subscribe((response: any) => {
    ...
    updateInductionConfig(config) {
        ...
        this.employeeService.upsertInductionConfiguration(inductionModel).subscribe((response: any) => {
            if (response.success == true) {
            }
        });
    }
        ...
        if (this.isValid) {
            this.userStoriesList.forEach((userStoryDetails) => {
                if (userStoryDetails.isShow) {
                    ...
                    this.employeeService.upsertAdhocWork(userStory).subscribe((result: any) => {
                        if (result.success) {
                        } else {
