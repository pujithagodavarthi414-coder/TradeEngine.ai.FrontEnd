...
@Component({
  selector: "app-billing-component-expenses",
...
export class ExpenseViewComponent extends AppBaseComponent {
  @ViewChildren("archiveExpensePopUp") archiveExpensePopover;
  @ViewChildren("approveExpensePopup") approveExpensePopOver;
  @ViewChildren("approveMultipleExpensesPopup") approveMultipleExpensesPopOver;
  ...
  set _dashboardName(data: string) {
    if (data != null && data !== undefined) {
      ...
      if (this.dashboardName == "All Expenses") {
        this.dashboardName = this.translateService.instant("EXPENSEMANAGEMENT.ALLEXPENSES");
      } else if (this.dashboardName == "Pending expenses") {
        this.dashboardName = this.translateService.instant("CLIENTS.PENDINGEXPENSES");
      } else if (this.dashboardName == "Approved expenses") {
        this.dashboardName = this.translateService.instant("CLIENTS.APPROVEDEXPENSES");
      }
    } else {
      this.dashboardName = this.translateService.instant("EXPENSEMANAGEMENT.ALLEXPENSES");
    }
  ...
  @Input("isPendingExpenses")
  set _isPendingExpenses(data: boolean) {
    this.isPendingExpenses = data;
    if (this.isPendingExpenses) {
      this.dashboardName = this.translateService.instant("CLIENTS.PENDINGEXPENSES");
    }
  }

  @Input("isApprovedExpenses")
  set _isApprovedExpenses(data: boolean) {
    this.isApprovedExpenses = data;
    if (this.isApprovedExpenses) {
      this.dashboardName = this.translateService.instant("CLIENTS.APPROVEDEXPENSES");
    }
  }
  ...
  isApprovedExpenses: boolean = false;
  isPendingExpenses: boolean = false;
  ...
  imageExtensions: string[] = ["png", "jpeg", "jpg", "gif", "PNG", "JPEG", "JPG", "GIF"];
  ...
  referenceId: string = ConstantVariables.EXPENSEREFENCEID;
  referenceTypeId: string = ConstantVariables.EXPENSEREFENCEID;
  ...
  searchExpenses() {
    ...
    expenseModel.isApprovedExpenses = this.isApprovedExpenses;
    expenseModel.isPendingExpenses = this.isPendingExpenses;
    ...
    expenseModel.searchText = this.searchText ? this.searchText.trim() : this.searchText;
  ...
  openCreateExpenseDialog(data: any) {
    if(data == null) {
      ...
      this.referenceId = data.expenseId;
    }
    ...
    const dialogRef = this.dialog.open(this.upsertExpenseComponent, {
      ...
      direction: "ltr",
      id: dialogId,
      data: { data, formPhysicalId: dialogId, referenceId: this.referenceId, referenceTypeId: this.referenceTypeId, moduleTypeId: this.moduleTypeId,isEditFieldPermission: this.canAccess_feature_CanSubmitCustomFieldsForExpenses},
      disableClose: false,
    });
  ...
  onCellClick(data) {
    ...
    const dialogRef = this.dialog.open(this.expenseDetailsComponent, {
      ...
      direction: "ltr",
      data: { data: data, isArchived: this.isArchived, formPhysicalId: dialogId },
      disableClose: false,
    });
  ...
  approveMultipleExpensePopupOpen(approveExpensePopup) {
    approveExpensePopup.openPopover();
  }
  ...
  closeapproveExpensesPopup() {
    this.approveMultipleExpensesPopOver.forEach((p) => p.closePopover());
  }
  ...
  shareExpense() {
    this.downloadExpense(this.expenseDetails);
  }

  downloadExpense(data) {
  ...
  approvePendingExpenses() {
    this.expenseService.approvePendingExpenses().subscribe((result: any) => {
      if (result.success) {
        this.closeapproveExpensesPopup();
        this.searchExpenses();
      } else {
  ...
  getExpenseStatuses() {
    this.expenseService.getExpenseStatuses(new ExpenseStatusModel()).subscribe((result: any) => {
      if (result.success) {
        ...
        let selectedStatusValues;
        if (this.isPendingExpenses) {
          let pendingStatus = this.statuses.find(x => x.expenseStatusName.toLowerCase() == 'pending');
          this.selectedStatus.push(pendingStatus.expenseStatusId);
          this.selectedStatusName = pendingStatus.expenseStatusName;
        }
        else if (this.isApprovedExpenses) {
          let approvedStatus = this.statuses.find(x => x.expenseStatusName.toLowerCase() == 'approved');
          this.selectedStatus.push(approvedStatus.expenseStatusId);
          this.selectedStatusName = approvedStatus.expenseStatusName;
        } else {
  ...
  resetAllFilters() {
    ...
    this.claimedBy = null;
    this.branchId = null;
    if (!this.isApprovedExpenses && !this.isPendingExpenses) {
      let selectedStatusValues;
      this.selectedStatus = [];
      this.statuses.forEach(element => {
        if (element.expenseStatusName.toLowerCase() != "paid") {
  ...
  searchCustomForms() {
    ...
    this.expenseService.searchCustomFields(customFormModel).subscribe((result) => {
  ...
  deleteCustomForm() {
    ...
    this.expenseService.upsertcustomField(customFieldModel).subscribe((result) => {
      if (result.success === true) {
        this.searchCustomForms();
      } else {
