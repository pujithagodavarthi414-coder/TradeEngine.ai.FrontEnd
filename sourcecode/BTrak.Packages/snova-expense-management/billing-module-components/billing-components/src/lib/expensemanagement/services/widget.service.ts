...
export class WidgetService {
	...
	GetCustomTags(customApplicationModel: CustomTagsModel) {
	  ...
	  getExpenseTabsBasedOnUser(widgetName: string) {
		let paramsobj = new HttpParams().set('WidgetName', widgetName);
		const httpOptions = {
		  headers: new HttpHeaders({ 'Content-Type': 'application/json' }),
		  params: paramsobj
		};
		return this._http.get(APIEndpoint + ApiUrls.ExpenseTabsBaseOnUser, httpOptions)
		  .pipe(map(result => {
			return result;
		  }));
	  }
