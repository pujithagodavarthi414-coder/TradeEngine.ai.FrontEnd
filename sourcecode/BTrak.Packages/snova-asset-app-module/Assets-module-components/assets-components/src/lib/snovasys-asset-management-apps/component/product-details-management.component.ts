...
export class ProductDetailsManagementComponent extends CustomAppBaseComponent {
    ...
    constructor(private assetService: AssetService, private toaster: ToastrService, private store: Store<State>,private router: Router) {
        super();
        this.page.size = 30;
    ...
    ngOnInit() {
        super.ngOnInit();
        if (this.router.url.includes('/assetmanagement/assetssettings')) {
            this.isPadding = true;
        }
    ...
    productSelected(productId, event) {
        if (productId == "all") {
    ...
    supplierSelected(supplierId, event) {
        if (supplierId == "all") {
            ...
            if(event == null)
              ...
              this.vendor = event.source.selected._element.nativeElement.innerText.trim();
        }
        else {
    ...
    filter() {
        if (this.searchText || this.searchProductCode || this.productName || this.searchManufactureCode || this.vendor) {
          return true;
        } else {
          return false;
        }
