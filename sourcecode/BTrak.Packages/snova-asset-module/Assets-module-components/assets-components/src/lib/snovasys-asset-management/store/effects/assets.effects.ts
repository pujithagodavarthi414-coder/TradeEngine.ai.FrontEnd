...
import {
  ...
  AssetsExceptionHandled, GetAssetByIdTriggered, GetAssetByIdCompleted, CreateAssetCompletedWithInPlaceUpdate, RefreshAssetsList, LoadAssetsFailed,
...
export class AssetEffects {
    ...
    switchMap((searchAction) => {
      ...
      return this.assetService
        ...
        .pipe(map((assetsList: any) => {
          ...
          catchError((error) => {
            return of(new AssetsExceptionHandled(error));
          })
        );
    })
  );
  ...
  selectedAssetAfterLoadAssets$: Observable<Action> = this.actions$.pipe(
    ...
    switchMap((searchAction) => {
      if (searchAction.assetsList.length > 0) {
        let data = searchAction.assetsList;
        return of(new GetSelectedAssetTriggered(data[0].assetId));
      } else {
  ...
  assetCountAfterLoadAssetsCompleted$: Observable<Action> = this.actions$.pipe(
    ...
    switchMap((searchAction) => {
      if (searchAction.assetsList.length > 0) {
        let data = searchAction.assetsList;
        return of(new AssetsTotalCountCompleted(data[0].totalCount));
      } else {
  ...
  selectedAssetAfterUpsertAsset$: Observable<Action> = this.actions$.pipe(
    ...
    switchMap((searchAction) => {
      if (searchAction.asset[0]) {
        let data = searchAction.asset[0];
        return of(new GetSelectedAssetTriggered(data.assetId));
      } else {
        return of(new GetSelectedAssetCompleted(""));
      }
    })
  ...
  @Effect()
  AssetsExceptionHandled$: Observable<Action> = this.actions$.pipe(
    ofType<AssetsExceptionHandled>(AssetsActionTypes.AssetsExceptionHandled),
    switchMap((searchAction) => {
