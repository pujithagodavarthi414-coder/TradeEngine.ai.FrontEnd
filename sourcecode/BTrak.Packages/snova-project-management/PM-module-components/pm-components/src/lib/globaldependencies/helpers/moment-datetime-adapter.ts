...
getMonthNames(style: 'long' | 'short' | 'narrow'): string[] {
    ...
    if (this.SUPPORTS_INTL_API) {
        ...
        return this.range(12, function (i) {
            return _sthis.stripDirectionalityCharacters(dtf_1.format(new Date(2017, i, 1)));
        });
    }
...
getDayOfWeekNames(style: 'long' | 'short' | 'narrow'): string[] {
    ...
    if (this.SUPPORTS_INTL_API) {
        ...
        return this.range(7, function (i) {
            return _thiss.stripDirectionalityCharacters(dtf_2.format(new Date(2017, 0, i + 1)));
        });
    }
...
format(date: Date, displayFormat: any): string {
    ...
    //     date.getFullYear();
    const dateWithoutTime = this.datePipe.transform(date, 'dd MMM yyyy hh:mm')
