...
export class TrackerScreenshotsComponent {
        ...
        if (data && data !== undefined) {
            ...
            if (trackerData) {
                ...
                if (trackerData.dialogId) {
                    ...
                    this.userStoryName = trackerData.userStoryName;
                }
    ...
    @Input("userStoryInputData")
    set _userStoryInputData(data: any) {
        if (data && data !== undefined) {
    ...
    isSelected: boolean;
    noData: boolean;
    pageSizeOptions: number[] = [5, 10, 15, 20, 25, 50, 100, 150, 200];
    pageSize: number = 5;
    pageNumber: number = 1;
    pageIndex: number = 0;
    totalCount: number;
    ...
    constructor(private masterDataManagementService: MasterDataManagementService, public dialog: MatDialog,private cdRef: ChangeDetectorRef) {
    ...
    getActTrackerUserActivityScreenshots() {
        ...
        this.masterDataManagementService.getActTrackerUserActivityScreenshots(this.webAppUsageSearch).subscribe((responseData: any) => {
    ...
    getActTrackerUserActivityScreenshotsByWorkItemId() {
        ...
        this.masterDataManagementService.getActTrackerUserActivityScreenshotsById(webAppUsageSearch).subscribe((responseData: any) => {
            if (responseData.success == true) {
                this.userActivityScreenshots = responseData.data;
                if (this.userActivityScreenshots.length == 0) {
                    this.noData = true;
    ...
    openModal(value) {
        (document.querySelector(".custom-modal-pm") as HTMLElement).style.display = "block";
        let i = 0;
    ...
    showSlides(n) {
        let i;
        (document.querySelector(".mySlides-pm") as HTMLElement).style.display = "none";
        this.screenImg = this.screen[this.slideIndex - 1].screenShotUrl;
        this.imageDetails = this.screen[this.slideIndex - 1];
        (document.querySelector(".mySlides-pm") as HTMLElement).style.display = "block";
    }
    ...
    closeModal() {
        (document.querySelector(".custom-modal-pm") as HTMLElement).style.display = "none";
    ...
    deleteScreenshots(formDirective: FormGroupDirective) {
        ...
        if (this.deleteForm.valid) {
            ...
            this.masterDataManagementService.deleteMultipleScreenshots(deleteScreenshot).subscribe((responseData: any) => {
