...
export class UserEffects {
  constructor(private actions$: Actions, private userService: UserService, private store$: Store<State>, private translateService: TranslateService) { }
  ...
  loadUsers$: Observable<Action> = this.actions$.pipe(
    ...
    switchMap(() => {
      return this.userService.GetAllUsers().pipe(
        ...
        catchError(err => {
          return of(new UserExceptionHandled(err));
        })
      );
    })
  );
    ...
    switchMap(CreateUsersTriggeredAction => {
      ...
      return this.userService
        ...
        .pipe(
          ...
          catchError(error => {
            return of(new UserExceptionHandled(error));
          })
        );
    })
  );
  ...
  LoggedUserId$: Observable<Action> = this.actions$.pipe(
    ...
    switchMap(searchAction => {
      return this.userService.getLoggedUserData().pipe(
  ...
  @Effect()
  UserExceptionHandled$: Observable<Action> = this.actions$.pipe(
    ofType<UserExceptionHandled>(
      UserActionTypes.UserExceptionHandled
    ),
    switchMap(searchAction => {
