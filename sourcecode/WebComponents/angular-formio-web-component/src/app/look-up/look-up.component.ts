...
export class LookUpComponent implements FormioCustomComponent<any> {
    ...
    constructor(private cdRef: ChangeDetectorRef, private formService: FormService
        , public dialog: MatDialog, private translateService: TranslateService, private datePipe: DatePipe,
                        ...
                        if (this.valueType == 'latest') {
                            ...
                            if (this.lookUpChildData) {
                                ...
                                keys.forEach((item, index) => {
                                    ...
                                    if (this.lookUpChildData[item + '-Type'] == 'address') {
                                        var addressObject = JSON.parse(this.lookUpChildData[item]);
                                        if(addressObject && addressObject.mode == "manual") {
                                            if (typeof addressObject.address === 'object') {
                                                // Manual implementation of Object.values()
                                                var formattedAddress = Object.keys(addressObject.address)
                                                    .map(key => addressObject.address[key])
                                                    .join(', ');
                                                this.lookUpChildData["formatted_address"] = formattedAddress;
                                                this.lookUpChildData["address1"] = addressObject.address.address1;
                                                this.lookUpChildData["address2"] = addressObject.address.address2;
                                                this.lookUpChildData["city"] = addressObject.address.city;
                                                this.lookUpChildData["state"] = addressObject.address.state;
                                                this.lookUpChildData["country"] = addressObject.address.country;
                                                this.lookUpChildData["zip"] = addressObject.address.zip;

                                            }
                                        }
                                        else if(addressObject && addressObject.mode == "autocomplete") {
                                            this.lookUpChildData["formatted_address"] = addressObject.formatted_address;   
                                        }
                                    }
                ...
                if (type == 'datetime' || type == 'mydatetime' || type == 'mylinkdatetime') {
                    ...
                    if (address && mode) {
                        if (mode == 'manual') {
                            ...
                            var addressCommaSeparatedString = [address1, address2, city, state, country, zip].join(", ");
                            return addressCommaSeparatedString;
                        }
                        else {
