...
export class DynamicHtmlCreationComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  constructor(private cd: ChangeDetectorRef, private sanitizer: DomSanitizer, private webPagesService: WebPagesService, public dialog: MatDialog
    ...
    console.log("app-dynamic-html-creation constructor");
  ...
  replaceDynamicFormTags(htmlTemplate) {
    ...
    if (customForm) {
      customForm.forEach((form) => {
        ...
        htmlTemplate = htmlTemplate.replace(form.tagName, '<app-dynamic-submit-form-component></app-dynamic-submit-form-component>');
      });
      if (this.templateTagStyles) {
