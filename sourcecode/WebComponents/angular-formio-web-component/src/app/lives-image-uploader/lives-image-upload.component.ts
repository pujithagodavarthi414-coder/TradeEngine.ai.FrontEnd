...
import { NgxGalleryImage, NgxGalleryOptions, NgxGalleryComponent, NgxGalleryImageSize } from "ngx-gallery-9";
...
@Component({
    selector: 'app-lives-image-upload',
    templateUrl: './lives-image-upload.component.html',
    styles: [`
      .custom-dropzone {
          height: 85px;
          border-radius: 5px;
...
export class LivesImageUploadComponent implements FormioCustomComponent<any> {
    ...
    filePatterns: any = '.jpg, .jpeg, .png, .gif, .bmp, .tiff, .webp, .svg';
    pdf: any;
    csv: any;
    office: any;
    disableds: boolean;
    ...
    folderReferenceId: string ='466ae1aa-6a42-414e-b5b6-3ddadc87d81f';
    ...
    fileExtensions: any='.jpg, .jpeg, .png, .gif, .bmp, .tiff, .webp, .svg';
    ...
    searchFiles() {
        if (this.folderReferenceId != null && this.folderReferenceId != '' && this.referenceTypeId != '' && this.referenceTypeId != null) {
            ...
            this.storeManagementService.searchFiles(this.folderReferenceId,this.referenceTypeId, null).subscribe((result: any) => {
                if (result.success) {
    ...
    getStoreConfigurations() {
        this.storeManagementService.getStoreConfiguration().subscribe((result: any) => {
            ...
            if (this.storeConfigurations) {
                ...
                this.fileExtensions = '.jpg, .jpeg, .png, .gif, .bmp, .tiff, .webp, .svg';
                ...
                if (response.success) {
                    ...
                    if (putBlocks.length == responseIndex) {
                        ...
                        this.storeManagementService.getBlobUrl(moduleTypeId, fileName, list, filetype, null).subscribe((response1: any) => {
                            ...
                            ser.progressValue = 0;
                            if (response1.success) {
                                //this.formioEvent.emit({ eventName: 'customEvent', data: { data: response1.data, type: 'livesimageupload', key: 'livesimageupload', selectionType: 'latest', value: response1.data } });
